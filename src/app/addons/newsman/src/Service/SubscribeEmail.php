...
class SubscribeEmail
{
     ...
     * @return array
     */
    public function execute($email, $firstname = '', $lastname = '', $ip = '', $props = array())
    {
        $this->logger->info(sprintf('Try to subscribe email %s', $email));

        $context = $this->client->createContext();
        $context->setEndpoint('subscriber.saveSubscribe');
        ...
        $this->logger->info(sprintf('Subscribed email %s', $email));
